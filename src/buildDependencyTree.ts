...
export const buildDependencyMap = (structureDefinitions: StructureDefinition[]) => {
    ...
    for (const structureDefinition of structureDefinitions) {
        ...
        if (!snapshot && !differential) {
            console.error(`StructureDefinition ${structureDefinition.id} has no snapshot or differential.`)
            continue
        }
        ...
        if (snapshotElements.length > 0) {
            dependencyMap = updateDependencyMap(id, dependencyMap, snapshotElements)
        }
        if (differentialElements.length > 0) {
            dependencyMap = updateDependencyMap(id, dependencyMap, differentialElements)
        }
    }
    return dependencyMap
}

const updateDependencyMap = (id: string, dependencyMap: DependencyMap, elements: ElementDefinition[]) => {
                ...
                if (typeInfo.extension) {
                    for (const typeExtension of typeInfo.extension) {
                        if (typeExtension.url === "http://hl7.org/fhir/StructureDefinition/structuredefinition-fhir-type") {
                            ...
                            if (!value) {
                                console.error(`StructureDefinition ${id} has no value for extension http://hl7.org/fhir/StructureDefinition/structuredefinition-fhir-type`)
                                continue
                            }
