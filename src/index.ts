...
export const main = async (inputFiles: string[], outputDir: string) => {
    ...
    for (const inputFile of inputFiles) {
        ...
        const classifiedDefinitions = await classifyFHIRDefinition(fhirDefinitions, {
            structureDefinitions: structureDefinitionRule
        });

        const fileStructureDefinitions = classifiedDefinitions['structureDefinitions'] || [];
        allStructureDefinitions.push(...fileStructureDefinitions);
    }

    const structureDefinitions = allStructureDefinitions;

    generateZodSchemaCodeFiles(structureDefinitions, outputDir, initializePrimitiveTypeSchemasCodes());
}
...
if (require.main === module) {
    ...
    if (inputFiles.length === 0) {
        console.error('Input file is required: fhir2zod -f <input-file> -o <output-dir>');
        process.exit(1);
    }
